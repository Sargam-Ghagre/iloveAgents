...
export default function RecommendationWizardStep({ stepIndex, preferences, errors, setPreference, toggleExtraPreference }) {
  ...
  if (stepIndex === 2) return <StepShell step="Step 3 of 5" title="Do you prefer a specific AI provider?" helper="Provider preference is a light ranking signal, not a strict filter."><div className="grid grid-cols-1 min-[420px]:grid-cols-2 gap-3">{PROVIDER_OPTIONS.map((option) => <OptionButton key={option.id} option={option} selected={preferences.providerPreference === option.id} onClick={() => setPreference('providerPreference', option.id)} />)}</div></StepShell>
  if (stepIndex === 3) return <StepShell step="Step 4 of 5" title="How should we balance cost and capability?" helper="This is preference guidance only; the registry does not include dollar-cost data."><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{BUDGET_OPTIONS.map((option) => <OptionButton key={option.id} option={option} selected={preferences.budgetPreference === option.id} onClick={() => setPreference('budgetPreference', option.id)} />)}</div></StepShell>
